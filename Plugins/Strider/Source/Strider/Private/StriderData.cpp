...
FSlopeLimbDefinition::FSlopeLimbDefinition()
	: LimbBoneCount(2),
	FootWidth(1.0f),
	LengthSqr(-1.0f),
	Length(-1.0f),
	...
	SlopePosition(FVector::ZeroVector),
	LastFootPosition(FVector::ZeroVector),
	FootVelocity(FVector::ZeroVector)
{
}

void FSlopeLimbDefinition::Initialize(const FBoneContainer& PoseBones)
{
	Length = -1.0f;
...
bool FSlopeLimbDefinition::IsValid(const FBoneContainer& PoseBones)
{
...
void FSlopeLimbDefinition::CalculateLength(FCSPose<FCompactPose>& Pose)
{
	//Length from foot to previous bone hinge
